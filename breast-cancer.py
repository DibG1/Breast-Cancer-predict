...
pickle_sc = open('stdsc_cancer1.pkl', 'rb')
StandardScaler = pickle.load(pickle_sc)
...
def prediction(mean_radius, mean_texture, mean_perimeter, mean_area,mean_smoothness):
    ...
    X=StandardScaler.fit_transform([[mean_radius, mean_texture, mean_perimeter, mean_area,mean_smoothness]])
    print(X)
    prediction = classifier.predict(X)
    if prediction==1:
        return "Patient is suffering from breast cancer"
    else:
        return "Patient is NOT suffering from breast cancer"
